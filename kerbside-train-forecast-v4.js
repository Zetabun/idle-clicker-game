...
function detailMarkup(result,date,options={}){const reasons=(result.reasons||[]).map(sentence).filter(Boolean),history=Number(result.historySamples)||0,mode=options.mode||(isFuture(date)?'Planning':'Live-adjusted'),meta=result.cancelled?'Service cancelled':`${result.confidence} confidence · Forecast v4 · ${mode}`;const detailReasons=Array.isArray(result.reasonDetail)&&result.reasonDetail.length?result.reasonDetail:(result.reasons||[]).map(text=>({text,direction:'flat'}));const flagLabel={up:'Busier',down:'Quieter',flat:'Context'};const tally={up:0,down:0,flat:0};const items=detailReasons.map(entry=>{const text=sentence(entry&&entry.text);if(!text)return '';const direction=entry&&flagLabel[entry.direction]?entry.direction:'flat';tally[direction]++;return `<li class="reason-${direction}"><span class="train-forecast-flag">${flagLabel[direction]}</span><span class="train-forecast-reason-text">${esc(text)}</span></li>`;}).filter(Boolean).join('');
/* Six equally weighted rows made you read all of them to learn which way the
   forecast was pushed. The balance is the first thing worth knowing, so it is
   stated once above the list instead of being inferred from the pills. */
const tallyParts=[tally.up?`${tally.up} busier`:'',tally.down?`${tally.down} quieter`:'',tally.flat?`${tally.flat} context`:''].filter(Boolean);
const tallyMarkup=tallyParts.length>1?`<span class="train-forecast-tally">${esc(tallyParts.join(' · '))}</span>`:'';const base=result.cancelled?'No crowding forecast is produced for a cancelled service. Its knock-on effect is still included in nearby trains.':'Forecast estimate only — no ticket sales, seat reservations or live carriage occupancy. Passenger-submitted crowding reports do not affect the score.';const method=options.note?`${base} ${options.note}`:base;const historyMarkup=history?`<div class="train-forecast-history"><span>Local history</span><b>${history} service observation${history===1?'':'s'} so far</b></div>`:'';
...
const probabilityMarkup=bands.length
  ?`<div class="train-forecast-probabilities"><span>Probability</span>`
    ...
    +`</div></div>`
  :'';return `<div class="train-forecast-head"><span class="train-forecast-status"><i></i><strong>${esc(result.label)}</strong></span><span class="train-forecast-meta">${esc(meta)}</span></div><div class="train-forecast-reasons"><span>Why this forecast</span>${tallyMarkup}<ul>${items}</ul></div><details class="train-forecast-method"><summary>How this is worked out</summary><p>${esc(method)}</p>${calibrationMarkup}</details>${probabilityMarkup}${historyMarkup}`;}
