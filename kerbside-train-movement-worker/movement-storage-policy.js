...
export const LIVE_RETENTION_MS=4*60*60*1000;
...
export function compactLiveSnapshot(s,{historyLimit=0}={}){const out=compactRecoverySnapshot(s);if(!out)return null;const limit=Math.max(0,Math.min(4,Math.floor(Number(historyLimit)||0))),history=limit&&Array.isArray(s.history)?s.history.slice(-limit):[];if(history.length)out.history=history;return out;}
