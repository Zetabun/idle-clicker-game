...
const planState={installed:false,active:false,from:null,to:null,results:[],preference:'balanced',constraints:{maxChanges:1,connectionBuffer:0},saved:[],savedFocus:null,eventsReady:false,events:[],filter:null,draftTouched:false,source:'',searchSeq:0,eventSeq:0,hidden:new Map(),observer:null};
...
function planEventDisplayRows(events,candidates,from,to,date){
...
async function searchPlanJourneys(options={}){
  ...
  if(!options||options.keepSavedFocus!==true)planState.savedFocus=null;planState.eventsReady=false;planState.events=[];
  ...
  try{
    ...
    const windowCandidates=(candidates||[]).filter(item=>{const minute=planCandidateDepartureMinute(item);return minute!=null&&minute>=startMinute&&minute<=endMinute;});
    ...
    const applyEventRows=(events,status={})=>{if(seq!==planState.searchSeq||eventSeq!==planState.eventSeq)return;const updated=planRankEnriched(enrichPlanCandidates(eligibleCandidates,from,to,date,events),planState.preference);planState.results=updated;planState.events=planEventDisplayRows(events,eligibleCandidates,from,to,date);planState.eventsReady=true;renderPlanResults(updated,{eventsReady:true});const pending=Number(status.pending)||0;planSetMessage(pending?`${planComparisonMessage(windowCandidates.length,eligibleCandidates.length)} Using the latest available information while event sources still update…`:planComparisonMessage(windowCandidates.length,eligibleCandidates.length,{eventsReady:true}));};
...
.plan-journey-surface{display:flex;flex-direction:column;min-height:0;overflow:hidden}.plan-results-head{flex:0 0 auto;overflow:visible;padding:18px 20px 14px;border-bottom:1px solid var(--rule);background:var(--ink-2)}
...
.plan-results-list{flex:1 1 auto;min-height:0;overflow-y:auto;padding:12px 16px 24px;overscroll-behavior:contain}
...
/* ------------------------------------------------------------------
   ...
   Saved journeys remain small journey locators. This layer gives them a
   dedicated workspace, refreshes them quietly against the current timetable,
   remembers the original planning intent, and records only a tiny resolved
   summary for change detection. Forecast/live evidence is always recalculated
   and is never persisted as authoritative saved state.
------------------------------------------------------------------ */
;(function(){
'use strict';
...
const AUTO_TICK_MS=15*60*1000;
const COVERAGE_TICK_MS=10*60*1000;
...
const STATUS={
  ...
  updated:{label:'Updated',tone:'updated'},
  'live-soon':{label:'Live soon',tone:'live'},
  today:{label:'Today',tone:'live'},
  delayed:{label:'Delayed',tone:'warn'},
  cancelled:{label:'Cancelled',tone:'danger'},
  alternative:{label:'Alternative found',tone:'warn'},
  expired:{label:'Expired',tone:'muted'},
  unavailable:{label:'Needs attention',tone:'warn'}
};
...
const planner=()=>window.__KERBSIDE_JOURNEY_PLANNER__||null;
const provider=()=>window.__KERBSIDE_TIMETABLE_PROVIDER__||null;
...
function dateDiff(from,to){const a=new Date(`${from}T12:00:00Z`),b=new Date(`${to}T12:00:00Z`);return Number.isNaN(a.getTime())||Number.isNaN(b.getTime())?0:Math.round((b-a)/86400000);}
function timeMinutes(value){const m=String(value||'').match(/^(\d{1,2}):(\d{2})$/);if(!m)return null;const h=Number(m[1]),n=Number(m[2]);return h>=0&&h<24&&n>=0&&n<60?h*60+n:null;}
function clock(value){const minute=Math.max(0,Math.min(1439,Math.round(Number(value)||0)));return `${String(Math.floor(minute/60)).padStart(2,'0')}:${String(minute%60).padStart(2,'0')}`;}
...
function relativeCheck(value){const stamp=Date.parse(value||'');if(!Number.isFinite(stamp))return'Not checked yet';const minutes=Math.max(0,Math.round((Date.now()-stamp)/60000));if(minutes<1)return'Checked just now';if(minutes<60)return`Checked ${minutes} min ago`;const hours=Math.round(minutes/60);if(hours<24)return`Checked ${hours}h ago`;return`Checked ${Math.round(hours/24)}d ago`;}
...
function selectorKey(value){const item=value||{};return String(item.uid||item.serviceID||item.trainId||item.std||'');}
function candidateServiceKey(row){if(!row)return'';if(row.journeyType==='connection'){const first=row.legs&&row.legs[0],onward=row.legs&&row.legs[1],change=String(row.interchange&&row.interchange.crs||'').toUpperCase();return `connection:${selectorKey(selector(first))}:${change}:${selectorKey(selector(onward))}`;}return `direct:${selectorKey(selector(row))}`;}
function savedServiceKey(saved){if(!saved)return'';if(saved.journeyType==='connection')return `connection:${selectorKey(saved.first)}:${String(saved.change||'').toUpperCase()}:${selectorKey(saved.onward)}`;return `direct:${selectorKey(saved.service)}`;}
function sourceLabel(source){return source==='darwin'?'Darwin':source==='network-rail'?'Network Rail SCHEDULE':'Official timetable';}
function constraintLabel(saved){const settings=saved&&saved.constraints||{},max=Number(settings.maxChanges)===0?0:1,buffer=Number(settings.connectionBuffer)||0;if(max===0)return'Direct only';return buffer?`Up to 1 change · +${buffer} min connection buffer`:'Up to 1 change · base connection minimum';}
function intentLabel(saved){const start=String(saved&&saved.searchStart||''),end=String(saved&&saved.searchEnd||''),windowText=start&&end?`${start}–${end}`:'saved time window',preference=PREF_LABELS[saved&&saved.preference]||'Balanced';return `${windowText} · ${preference} · ${constraintLabel(saved)}`;}
function readRawSaved(){try{const value=JSON.parse(localStorage.getItem(SAVED_KEY)||'[]');return Array.isArray(value)?value:[];}catch(error){return[];}}
function readMeta(){try{const value=JSON.parse(localStorage.getItem(META_KEY)||'{}');return {entries:value&&value.entries&&typeof value.entries==='object'?value.entries:{},coverageKey:String(value&&value.coverageKey||''),updatedAt:String(value&&value.updatedAt||'')};}catch(error){return {entries:{},coverageKey:'',updatedAt:''};}}
function writeMeta(){state.meta.updatedAt=new Date().toISOString();try{localStorage.setItem(META_KEY,JSON.stringify(state.meta));}catch(error){}}
function baselineFromSaved(saved,source=''){return {departure:String(saved&&saved.scheduledDeparture||''),arrival:String(saved&&saved.scheduledArrival||''),journeyType:saved&&saved.journeyType==='connection'?'connection':'direct',changes:saved&&saved.journeyType==='connection'?1:0,source:String(source||''),serviceKey:savedServiceKey(saved)};}
function ensureMeta(saved,sourceHint=''){
  if(!saved||!saved.id)return null;let item=state.meta.entries[saved.id];if(!item||typeof item!=='object')item={};
  ...
  item.lastResolved=item.lastResolved&&typeof item.lastResolved==='object'?item.lastResolved:null;
  item.lastChecked=String(item.lastChecked||'');item.status=String(item.status||'planned');item.resolution=String(item.resolution||'');item.source=String(item.source||sourceHint||'');item.changes=Array.isArray(item.changes)?item.changes:[];item.lastError=String(item.lastError||'');
  state.meta.entries[saved.id]=item;return item;
}
function syncSaved({sourceHint='',render=true}={}){
  const api=planner(),rows=api&&typeof api.readSavedJourneys==='function'?api.readSavedJourneys():readRawSaved();
  state.saved=Array.isArray(rows)?rows:[];const ids=new Set(state.saved.map(item=>item.id));let changed=false;
  for(const saved of state.saved){if(!state.meta.entries[saved.id]){ensureMeta(saved,sourceHint);changed=true;}}
  for(const id of Object.keys(state.meta.entries)){if(!ids.has(id)){delete state.meta.entries[id];state.live.delete(id);changed=true;}}
  if(api&&api.planState)api.planState.saved=state.saved;
  if(changed)writeMeta();if(render)renderSavedView();return state.saved;
}
function savedWindow(saved){const dep=timeMinutes(saved&&saved.scheduledDeparture),storedStart=timeMinutes(saved&&saved.searchStart),storedEnd=timeMinutes(saved&&saved.searchEnd);if(dep==null)return {start:saved&&saved.searchStart||'09:00',end:saved&&saved.searchEnd||'11:00'};let start=storedStart==null?Math.max(0,dep-30):Math.min(storedStart,Math.max(0,dep-30)),end=storedEnd==null?Math.min(1439,dep+45):Math.max(storedEnd,Math.min(1439,dep+45));if(end<=start)end=Math.min(1439,start+120);return {start:clock(start),end:clock(end)};}
...
function updateLocator(saved,row){const connection=row&&row.journeyType==='connection',first=connection&&row.legs&&row.legs[0],onward=connection&&row.legs&&row.legs[1];return {...saved,v:1,refreshedAt:new Date().toISOString(),journeyType:connection?'connection':'direct',service:selector(row),first:connection?selector(first):{serviceID:'',uid:'',trainId:'',std:''},onward:connection?selector(onward):{serviceID:'',uid:'',trainId:'',std:''},change:connection?String(row.interchange&&row.interchange.crs||'').toUpperCase():'',scheduledDeparture:String(row.std||row.departure||''),scheduledArrival:String(row.arrival||'')};}
function replaceLocator(saved,row){const raw=readRawSaved(),next=updateLocator(saved,row),updated=raw.map(item=>String(item&&item.id||'')===saved.id?next:item);try{localStorage.setItem(SAVED_KEY,JSON.stringify(updated));}catch(error){}const api=planner();if(api&&api.planState&&typeof api.readSavedJourneys==='function')api.planState.saved=api.readSavedJourneys();state.saved=api&&typeof api.readSavedJourneys==='function'?api.readSavedJourneys():updated;return state.saved.find(item=>item.id===saved.id)||next;}
function forecastCandidate(row,saved){const api=window.__KERBSIDE_FORECAST_V4__||window.__KERBSIDE_FORECAST_V3__;if(!api||typeof api.forecast!=='function')return {...row,forecast:{primary:{score:2,label:'Moderate',level:'moderate',confidence:'Low',probabilities:{quiet:.25,moderate:.25,busy:.25,veryBusy:.25},reasons:[]},score:2}};const date=new Date(`${saved.date}T12:00:00`),interchange=row&&row.interchange?{crs:String(row.interchange.crs||''),name:String(row.interchange.name||row.interchange.locationName||row.interchange.crs||'')}:null,eventJourney={origin:saved.from.name,originCrs:saved.from.crs,destination:saved.to.name,destinationCrs:saved.to.crs,interchanges:interchange?[interchange.name||interchange.crs]:[],date:saved.date};if(row&&row.journeyType==='connection'&&Array.isArray(row.legs)&&row.legs.length){const forecasts=row.legs.map((leg,index)=>api.forecast(leg,index,row.legs,{station:index===0?saved.from:interchange||saved.from,referenceDate:date,eventJourney,messages:[]})),worst=forecasts.reduce((best,item)=>!best||Number(item&&item.score)>Number(best&&best.score)?item:best,null)||forecasts[0];return {...row,forecast:{primary:worst,legs:forecasts,score:Math.max(...forecasts.map(item=>Number(item&&item.score)||0))}};}const result=api.forecast(row,0,[row],{station:saved.from,referenceDate:date,eventJourney,messages:[]});return {...row,forecast:{primary:result,legs:[result],score:Number(result&&result.score)||2}};}
function chooseAlternative(saved,candidates){const list=Array.isArray(candidates)?candidates:[];if(!list.length)return null;const api=planner(),enriched=list.map(row=>forecastCandidate(row,saved));if(api&&typeof api.planRankEnriched==='function')return api.planRankEnriched(enriched,saved.preference||'balanced')[0]||null;return enriched.slice().sort((a,b)=>(Number(a.departureMinute)||0)-(Number(b.departureMinute)||0))[0]||null;}
async function liveEvidence(saved,row){if(!saved||saved.date!==todayLondon())return null;const overlay=window.__KERBSIDE_TRAIN_OVERLAY__;if(!overlay||typeof overlay.refresh!=='function'||typeof overlay.evidenceFor!=='function')return null;try{await overlay.refresh({crs:saved.from.crs,date:saved.date,force:true});const target=row&&row.journeyType==='connection'&&row.legs&&row.legs[0]?row.legs[0]:row;return overlay.evidenceFor(target)||null;}catch(error){return null;}}
function delayedEvidence(live,scheduled){if(!live)return false;const etd=String(live.etd||'').trim();if(/^delayed$/i.test(etd)||live.delayReason)return true;const expected=timeMinutes(etd),std=timeMinutes(scheduled);return expected!=null&&std!=null&&expected>std+1;}
function changesSinceSaved(meta,current,resolution){const base=meta&&meta.baseline||{},changes=[];if(resolution==='alternative'||resolution==='closest')changes.push(`Original service is no longer available as saved; ${current.departure||'a nearby option'} is the best current match in your saved window.`);if(base.departure&&current.departure&&base.departure!==current.departure)changes.push(`Departure changed from ${base.departure} to ${current.departure}.`);if(base.arrival&&current.arrival&&base.arrival!==current.arrival)changes.push(`Arrival changed from ${base.arrival} to ${current.arrival}.`);if(Number.isFinite(Number(base.changes))&&Number(base.changes)!==Number(current.changes))changes.push(`Changes changed from ${base.changes} to ${current.changes}.`);const previousSource=meta&&meta.lastResolved&&meta.lastResolved.source||base.source||'';if(previousSource==='network-rail'&&current.source==='darwin')changes.push('Now covered by Darwin; Kerbside upgraded this save from the long-range Network Rail timetable.');return changes;}
function statusFor(saved,current,resolution,changes,live){const today=todayLondon(),diff=dateDiff(today,saved.date);if(diff<0)return'expired';if(live&&live.isCancelled)return'cancelled';if(diff===0&&delayedEvidence(live,current&&current.departure))return'delayed';if(resolution==='alternative'||resolution==='closest')return'alternative';const material=(changes||[]).some(text=>!/Now covered by Darwin/.test(text));if(material)return'updated';if(diff===0)return'today';if(current&&current.source==='darwin')return'live-soon';return'planned';}
function liveText(live){if(!live)return'';if(live.isCancelled)return live.cancelReason?`Cancelled · ${live.cancelReason}`:'Cancelled';const bits=[];if(live.etd)bits.push(live.etd==='On time'?'On time':`Expected ${live.etd}`);if(live.platform)bits.push(`Platform ${live.platform}`);return bits.join(' · ');}
function refreshAgeLimit(saved){const diff=dateDiff(todayLondon(),saved.date);if(diff<0)return Infinity;if(diff===0)return 5*60*1000;if(diff<=2)return 15*60*1000;return 6*60*60*1000;}
...
async function startActiveSavedJourney(id,{stayInSaved=false}={}){
...
async function refreshSavedJourney(id,{force=false,reason='auto'}={}){
  syncSaved({render:false});const saved=state.saved.find(item=>item.id===String(id||''));if(!saved)return false;const meta=ensureMeta(saved);if(!shouldRefresh(saved,meta,force)){renderSavedView();return true;}if(state.refreshing.has(saved.id))return false;state.refreshing.add(saved.id);renderSavedView();
  try{
    const api=provider(),plan=planner();if(!api)throw new Error('Timetable provider is still loading.');
    const windowValue=savedWindow(saved),getter=typeof api.getJourneyOptions==='function'?api.getJourneyOptions.bind(api):api.getServices&&api.getServices.bind(api);if(!getter)throw new Error('Journey timetable is unavailable.');
    const rows=await getter({from:saved.from.crs,to:saved.to.crs,date:saved.date,departAfter:windowValue.start,departBefore:windowValue.end,maxResults:MAX_CANDIDATES});
    const source=String(rows&&rows.kerbsideSource||window.__KERBSIDE_TRAIN_TIMETABLE__?.state?.scheduleSource||meta.source||'');
    const eligible=plan&&typeof plan.planFilterCandidates==='function'?plan.planFilterCandidates(rows,saved.constraints):Array.from(rows||[]);
    let match=plan&&typeof plan.planMatchSavedJourney==='function'?plan.planMatchSavedJourney(saved,eligible,{allowClosest:false}):null,resolution=match&&match.confidence||'',candidate=match&&match.candidate||null;
    if(!candidate){const close=plan&&typeof plan.planMatchSavedJourney==='function'?plan.planMatchSavedJourney(saved,eligible,{allowClosest:true}):null;if(close&&close.candidate){candidate=close.candidate;resolution='closest';}else{candidate=chooseAlternative(saved,eligible);resolution=candidate?'alternative':'missing';}}
    meta.lastChecked=new Date().toISOString();meta.lastError='';meta.source=source;meta.resolution=resolution;
    if(!candidate){meta.lastResolved=null;meta.changes=['The original service could not be found in the current timetable and no suitable alternative met the saved journey constraints.'];meta.status=dateDiff(todayLondon(),saved.date)<0?'expired':'unavailable';state.live.delete(saved.id);writeMeta();renderSavedView();return false;}
    const current=summaryFromCandidate(candidate,source),live=await liveEvidence(saved,candidate);meta.changes=changesSinceSaved(meta,current,resolution);meta.status=statusFor(saved,current,resolution,meta.changes,live);meta.lastResolved=current;state.live.set(saved.id,live);
    if(resolution==='exact'||resolution==='identity')replaceLocator(saved,candidate);
    writeMeta();renderSavedView();return true;
  }catch(error){meta.lastChecked=new Date().toISOString();meta.lastError=error&&error.message?error.message:'Refresh unavailable';meta.status=dateDiff(todayLondon(),saved.date)<0?'expired':'unavailable';writeMeta();renderSavedView();return false;}
  finally{state.refreshing.delete(saved.id);renderSavedView();}
}
async function refreshAll({force=false,reason='auto',ids=null}={}){if(state.refreshAllPromise)return state.refreshAllPromise;syncSaved({render:false});const targets=(Array.isArray(ids)&&ids.length?ids:state.saved.map(item=>item.id)).filter(Boolean);if(!targets.length)return false;state.refreshAllPromise=(async()=>{for(let i=0;i<targets.length;i+=2)await Promise.all(targets.slice(i,i+2).map(id=>refreshSavedJourney(id,{force,reason})));state.lastRefreshAt=Date.now();syncSaved();return true;})().finally(()=>{state.refreshAllPromise=null;renderSavedView();});return state.refreshAllPromise;}
function coverageKey(manifest){const sources=manifest&&manifest.sources||{},darwin=sources.darwin||{},network=sources.networkRail||{};return [darwin.timetableId||'',darwin.dates&&darwin.dates[darwin.dates.length-1]||'',network.timetableId||'',network.dates&&network.dates[network.dates.length-1]||''].join('|');}
async function checkCoverageSnapshot({force=false}={}){syncSaved({render:false});if(!state.saved.length)return false;const api=provider();if(!api||typeof api.getCoverage!=='function')return false;try{const manifest=await api.getCoverage({force:true}),key=coverageKey(manifest),previous=state.meta.coverageKey;state.meta.coverageKey=key;writeMeta();if(previous&&key&&previous!==key)await refreshAll({force:true,reason:'snapshot'});else if(force)await refreshAll({force:true,reason:'coverage'});return true;}catch(error){return false;}}
function statusMarkup(status){const item=STATUS[status]||STATUS.planned;return `<span class="saved-v2-status saved-v2-${esc(item.tone)}">${esc(item.label)}</span>`;}
...
function snapshotHidden(root,keep){if(!root)return;for(const child of [...root.children]){if(keep.has(child))continue;if(!state.hidden.has(child))state.hidden.set(child,!!child.hidden);child.hidden=true;}}
function restoreHidden(){for(const [element,hidden] of state.hidden){if(element&&element.isConnected)element.hidden=hidden;}state.hidden.clear();}
function exitSavedView(){if(!state.active)return;restoreHidden();const side=$('savedJourneySidebar'),surface=$('savedJourneySurface');if(side)side.hidden=true;if(surface)surface.hidden=true;state.active=false;}
...
function installStyles(){if($('kerbsideSavedJourneysV2Styles'))return;const style=document.createElement('style');style.id='kerbsideSavedJourneysV2Styles';style.textContent=`
.train-view-tabs{grid-template-columns:repeat(3,minmax(0,1fr))!important}.train-view-tabs [data-train-view="saved"]{position:relative}.saved-v2-tab-count{display:inline-grid;place-items:center;min-width:16px;height:16px;margin-left:3px;padding:0 4px;border-radius:999px;background:rgb(var(--led-rgb) / .18);font-size:8px}.saved-v2-tab-count:empty{display:none}
.saved-v2-sidebar[hidden],.saved-v2-surface[hidden]{display:none!important}.saved-v2-sidebar{display:grid;gap:12px}.saved-v2-sidebar h2{margin:0;font-size:24px}.saved-v2-sidebar p{margin:0;color:var(--text-dim);font-size:11px;line-height:1.5}.saved-v2-summary{display:grid;grid-template-columns:1fr auto;gap:8px;align-items:center;padding:10px;border:1px solid var(--rule);border-radius:10px;background:var(--ink-2)}.saved-v2-summary span{color:var(--text-dim);font-size:10px}.saved-v2-summary strong{font-family:'Martian Mono',monospace;font-size:18px;color:var(--led)}.saved-v2-refresh-all{min-height:44px;border:1px solid var(--led);border-radius:10px;background:var(--led);color:var(--on-accent);font-weight:800}.saved-v2-refresh-all:disabled{opacity:.6}.saved-v2-auto{color:var(--text-mute)!important;font-size:9.5px!important}
.saved-v2-surface{display:flex;flex-direction:column;min-height:0}.saved-v2-head{padding:18px 20px 14px;border-bottom:1px solid var(--rule);background:var(--ink-2)}.saved-v2-head h2{margin:5px 0 0;font-size:21px}.saved-v2-head p{margin:4px 0 0;color:var(--text-dim);font-size:11px}.saved-v2-list{flex:1;min-height:0;overflow-y:auto;padding:12px 16px 24px;display:grid;align-content:start;gap:10px}.saved-v2-card{display:grid;gap:10px;padding:14px;border:1px solid var(--rule);border-radius:12px;background:var(--ink-2)}.saved-v2-card>header{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.saved-v2-card h3{margin:3px 0 0;font-size:14px}.saved-v2-date{color:var(--text-dim);font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.06em}.saved-v2-status{flex:0 0 auto;padding:4px 7px;border:1px solid var(--rule);border-radius:999px;font-size:9px;font-weight:800}.saved-v2-planned,.saved-v2-muted{color:var(--text-dim)}.saved-v2-updated,.saved-v2-live{color:var(--led);border-color:rgb(var(--led-rgb) / .35)}.saved-v2-warn{color:var(--warn)}.saved-v2-danger{color:var(--warn);font-weight:900}.saved-v2-times{display:flex;gap:10px;align-items:baseline;flex-wrap:wrap}.saved-v2-times strong{font-family:'Martian Mono',monospace;font-size:13px}.saved-v2-times span{color:var(--text-dim);font-size:10px}.saved-v2-live{padding:8px 9px;border:1px solid rgb(var(--led-rgb) / .24);border-radius:8px;background:rgb(var(--led-rgb) / .06);font-size:10px;font-weight:800}.saved-v2-events{display:grid;gap:6px;padding:9px;border:1px solid rgb(var(--led-rgb) / .22);border-radius:9px;background:rgb(var(--led-rgb) / .04)}.saved-v2-events>span{color:var(--led);font-size:8.5px;font-weight:800;letter-spacing:.07em;text-transform:uppercase}.saved-v2-events>div{display:flex;gap:6px;flex-wrap:wrap}.saved-v2-event{display:grid;gap:2px;min-width:170px;padding:7px 8px;border:1px solid var(--rule);border-radius:8px;background:var(--ink)}.saved-v2-event b{font-size:10px}.saved-v2-event small{color:var(--text-dim);font-size:9px;line-height:1.35}.saved-v2-intent,.saved-v2-changes-wrap{display:grid;gap:4px;padding-top:8px;border-top:1px solid var(--rule)}.saved-v2-intent>span,.saved-v2-changes-wrap>span{color:var(--text-mute);font-size:8.5px;font-weight:800;letter-spacing:.07em;text-transform:uppercase}.saved-v2-intent strong{font-size:10px;line-height:1.45}.saved-v2-changes{display:grid;gap:3px;margin:0;padding-left:17px;color:var(--text-dim);font-size:10px;line-height:1.45}.saved-v2-changes li::marker{color:var(--led)}.saved-v2-nochange{margin:0;color:var(--text-dim);font-size:10px;line-height:1.45}.saved-v2-error{padding:8px;border-radius:8px;background:rgb(var(--warn-rgb,255 166 0) / .08);color:var(--warn);font-size:10px}.saved-v2-card>footer{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap}.saved-v2-card>footer>span{color:var(--text-mute);font-size:9px}.saved-v2-card>footer>div{display:flex;gap:6px;flex-wrap:wrap}.saved-v2-card button{min-height:34px;padding:7px 9px;border:1px solid var(--rule);border-radius:8px;background:var(--ink);color:var(--text);font-size:9.5px;font-weight:800}.saved-v2-card button[data-saved-v2-open]{color:var(--led)}.saved-v2-card .saved-v2-active{border-color:rgb(var(--live-rgb) / .38);background:rgb(var(--live-rgb) / .07);color:var(--live)}.saved-v2-card .saved-v2-follow{border-color:rgb(var(--led-rgb) / .38);background:rgb(var(--led-rgb) / .07);color:var(--led)}.saved-v2-card .saved-v2-follow[aria-pressed="true"]{border-color:rgb(var(--live-rgb) / .42);background:rgb(var(--live-rgb) / .08);color:var(--live)}.saved-v2-card button:disabled{opacity:.58;cursor:wait}.saved-v2-card .saved-v2-remove{color:var(--text-dim)}.saved-v2-empty{display:grid;gap:4px;padding:28px 18px;text-align:center;color:var(--text-dim)}.saved-v2-empty strong{color:var(--text);font-size:14px}.saved-v2-empty span{font-size:11px;line-height:1.5}
@media(max-width:820px){.saved-v2-sidebar{gap:9px}.saved-v2-sidebar h2{font-size:18px}.saved-v2-surface{flex:0 0 auto}.saved-v2-head{padding:11px 12px 9px}.saved-v2-head h2{font-size:17px}.saved-v2-list{overflow:visible;padding:8px 9px calc(18px + env(safe-area-inset-bottom))}.saved-v2-card{padding:11px}.saved-v2-card>footer{align-items:flex-start;flex-direction:column}.saved-v2-card>footer>div{width:100%}.saved-v2-card>footer button{flex:1 1 auto}}
@media(max-width:430px){.train-view-tabs button{padding:7px 5px;font-size:10px}.saved-v2-card>header{gap:8px}.saved-v2-card h3{font-size:12px}.saved-v2-times strong{font-size:12px}}
`;document.head.appendChild(style);}
function install(){const api=planner(),tabs=$('trainViewTabs'),sidebar=document.querySelector('.train-sidebar'),content=document.querySelector('.train-content');if(!api||!api.planState||!api.planState.installed||!tabs||!sidebar||!content)return false;if(state.installed)return true;installStyles();
  const legacy=$('planSavedPanel');if(legacy)legacy.remove();
  const button=document.createElement('button');button.type='button';button.setAttribute('role','tab');button.dataset.trainView='saved';button.setAttribute('aria-selected','false');button.setAttribute('aria-pressed','false');button.innerHTML='Saved journeys <span id="savedJourneyTabCount" class="saved-v2-tab-count"></span>';tabs.appendChild(button);
  const side=document.createElement('section');side.id='savedJourneySidebar';side.className='saved-v2-sidebar';side.hidden=true;side.innerHTML='<div class="train-kicker">Saved journeys</div><h2>Your journeys</h2><p>Kerbside keeps upcoming saves aligned with the latest timetable while preserving the plan you originally chose.</p><div class="saved-v2-summary"><span>Saved journeys</span><strong id="savedJourneySidebarCount">0</strong></div><button id="savedJourneyRefreshAll" class="saved-v2-refresh-all" type="button">Refresh all</button><p id="savedJourneyAutoMeta" class="saved-v2-auto"></p>';sidebar.insertBefore(side,$('trainPlanner'));
  const surface=document.createElement('section');surface.id='savedJourneySurface';surface.className='saved-v2-surface';surface.hidden=true;surface.innerHTML='<header class="saved-v2-head"><div class="train-kicker">Saved journeys</div><h2>Upcoming and saved trips</h2><p><span id="savedJourneyCount">0</span> saved · timetable changes are compared with what you originally chose.</p></header><div id="savedJourneyList" class="saved-v2-list"></div>';content.appendChild(surface);
  button.addEventListener('click',enterSavedView);for(const view of tabs.querySelectorAll('[data-train-view="trains"],[data-train-view="plan"]'))view.addEventListener('click',()=>{if(state.active)exitSavedView();},true);
  $('savedJourneyRefreshAll').addEventListener('click',async event=>{event.currentTarget.disabled=true;try{await refreshAll({force:true,reason:'manual-all'});}finally{event.currentTarget.disabled=false;}});
  ...
  document.addEventListener('click',event=>{const save=event.target&&event.target.closest&&event.target.closest('#planJourneyResults [data-plan-save-key]');if(!save)return;const before=new Set(state.saved.map(item=>item.id));setTimeout(()=>{const rows=syncSaved({sourceHint:api.planState.source||''}),added=rows.filter(item=>!before.has(item.id));for(const item of added)ensureMeta(item,api.planState.source||'');writeMeta();const message=$('planJourneyMessage');if(added.length&&message)message.textContent='Journey saved. Kerbside will keep it up to date automatically and recalculate Forecast v4 whenever it refreshes.';if(added.length)refreshAll({force:true,reason:'saved',ids:added.map(item=>item.id)});},30);});
  window.addEventListener('storage',event=>{if(event.key===SAVED_KEY||event.key===META_KEY){state.meta=readMeta();syncSaved();}});window.addEventListener('focus',()=>{checkCoverageSnapshot();refreshAll({reason:'focus'});});document.addEventListener('visibilitychange',()=>{if(!document.hidden){checkCoverageSnapshot();refreshAll({reason:'visible'});}});
  state.meta=readMeta();state.installed=true;syncSaved();setTimeout(()=>{checkCoverageSnapshot();refreshAll({force:true,reason:'open'});},500);state.timer=setInterval(()=>{if(!document.hidden)refreshAll({reason:'timer'});},AUTO_TICK_MS);state.coverageTimer=setInterval(()=>{if(!document.hidden)checkCoverageSnapshot();},COVERAGE_TICK_MS);return true;
}
function init(){if(!install())setTimeout(init,60);}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init,{once:true});else init();
