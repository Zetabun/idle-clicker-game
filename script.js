(function() {
  ...
  function drawCarCanvas() {
    ...
    // When car is at 0 miles or out of fuel, effectiveSpeed becomes 0 so background and loot stop.
  ...
  // Update loot position based on car direction. Called in both branches.
  function updateRoadLoot(deltaTime, effectiveSpeed) {
    for (let i = game.roadLoot.length - 1; i >= 0; i--) {
      const loot = game.roadLoot[i];
      // Multiply by car.direction: if direction is 1 (forward), loot moves left; if -1, loot moves right.
      loot.x -= effectiveSpeed * game.car.direction * deltaTime * 50;
      // When the car is returning, remove loot if it goes off the right edge.
  ...
  canvas.addEventListener("click", function(e) {
    ...
    // Increase hit area to 25 pixels for more reliable loot clicks
  ...
  clickButton.addEventListener("click", function() {
    const amount = game.clickValue * game.clickMultiplier * game.prestige.multiplier;
    game.aether += amount;
  ...
  function gameLoop() {
    ...
    if (game.car.isStuck) {
      ...
      if (game.car.direction === -1) {
        ...
        // Continue to update loot movement while returning until car reaches 0 miles.
  ...
  function gameLoop() {
    ...
    if (game.car.isStuck) {
      ...
      if (game.car.direction === -1) {
        ...
        game.car.miles = Math.max(game.car.miles - milesThisFrame, 0);
        game.car.environmentOffset -= effectiveSpeed * deltaTime * 50;
        // Update loot even when returning
        updateRoadLoot(deltaTime, effectiveSpeed);
        if (game.car.miles === 0 && game.trunk.items.length > 0) {
          showEventMessage("Loot dropped off to the garage.");
          game.garage = game.garage.concat(game.trunk.items);
  ...
  function gameLoop() {
    ...
    if (game.car.isStuck) {
      ...
      if (game.car.direction === -1) {
        ...
        game.car.miles = Math.max(game.car.miles - milesThisFrame, 0);
        game.car.environmentOffset -= effectiveSpeed * deltaTime * 50;
        // Update loot movement even while returning
        updateRoadLoot(deltaTime, effectiveSpeed);
        if (game.car.miles === 0 && game.trunk.items.length > 0) {
          showEventMessage("Loot dropped off to the garage.");
          game.garage = game.garage.concat(game.trunk.items);
