(function() {
  ...
  function drawEnvironment() {
    // For Neon City, use environmentHistory if available.
  ...
  function drawBgItems() {
    ...
    // For non-Neon City environments, use mod-based drawing.
    ...
    if (env.name === "City") {
      ...
      if (currentNeonCityEnv !== "Neon City") {
        ...
        // Initialize environmentHistory if not already.
  ...
  // Added randomized window "off" effect: some windows are drawn dark gray.
  function drawNeonBuilding(x, baseY, buildingWidth, buildingHeight) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        // Randomly decide if the window light is off (20% chance)
        if (Math.random() < 0.2) {
          ctx.fillStyle = "#333333"; // dark grey for "off" window
        } else {
          ctx.fillStyle = "#ffff00"; // bright yellow for "on" window
        }
  ...
  function gameLoop() {
    ...
    // Spawn loot if traveling forward.
    ...
    // Offline progression & movement simulation:
    if (game.car.isStuck) {
      ...
      const direction = game.car.direction; // 1 for forward, -1 for backward
      ...
      // Environment history management (for Neon City):
      if (ENVIRONMENTS[game.car.environmentIndex].name === "Neon City") {
        // When moving forward, record new environment segments at every 50-mile boundary.
        ...
        // When moving backward, if we fall below the start of the last segment, revert.
        ...
        // For non-Neon City environments, we simply update the environmentOffset.
      }
  ...
  function drawNeonBuilding(x, baseY, buildingWidth, buildingHeight) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        let wx = x + windowPaddingX + col * (windowWidth + windowPaddingX);
        let wy = (baseY - buildingHeight) + windowPaddingY + row * (windowHeight + windowPaddingY);
        // Randomly decide if the window is "off"
        if (Math.random() < 0.2) {
          ctx.fillStyle = "#333333"; // dark gray for off window
        } else {
  ...
  function gameLoop() {
    ...
    if (game.car.isStuck) {
      ...
      const direction = game.car.direction; // 1 forward, -1 backward
      let effectiveSpeed = game.car.speed * game.car.tempSpeedModifier;
      const milesWanted = effectiveSpeed * deltaTime;
      const consumptionRate =
        game.car.baseFuelConsumption * (1 - game.car.efficiencyUpgrade.level * game.car.efficiencyUpgrade.efficiencyBonus);
      ...
      // Environment persistence for Neon City:
      if (ENVIRONMENTS[game.car.environmentIndex].name === "Neon City") {
        if (direction === 1 && Math.floor(game.car.miles / 50) !== Math.floor(lastEnvChangeMiles / 50)) {
          let newEnv;
          do {
