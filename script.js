...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Environment definitions (order is important)
  const ENVIRONMENTS = [
  ...
  // Environment flavor comments for log messages
  const ENV_COMMENTS = {
  ...
  // Timers for weather and snow-stuck checks
  let weatherTimer = 0;
  ...
  function openInventory() {
    // Update inventory display (for now, only the car colour)
  ...
  function updateBgItems(deltaTime, effectiveSpeed) {
    // Scroll items at the same rate as the background (multiplied by 50)
  ...
  ========================= */
  // (formatNumber already defined above)

  /* =========================
       INVENTORY (handled in overlay via index.html)
  ========================= */
  // (Overlay functions were defined earlier above)
  ...
  function loadGame() {
    ...
    if (savedGame) {
      ...
      // New game: randomize weather and environment once
  ...
  function updateWeather(deltaTime) {
    ...
    // If weather is Snow and no snow tyres, check for getting stuck
  ...
  function drawCarCanvas() {
    ...
    // Left HUD: Miles / Environment / Weather
    const envName = ENVIRONMENTS[game.car.environmentIndex].name;
    ctx.font = "16px Arial";
    let currentWeather = WEATHERS[game.car.weatherIndex].name;
    ...
    // Bobbing effect (only if car is moving and has fuel)
  ...
  // Draw the car – if Car Paint is unlocked, use its color; otherwise default
    ...
    if (game.car.isStuck) {
      ...
      if (game.car.fuel > 0) {
        ...
        if (milesThisFrame > 0) {
          ...
          // Change environment every 50 miles
