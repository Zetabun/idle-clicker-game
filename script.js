(function() {
  ...
  function drawCarCanvas() {
    ...
    // When at the garage (0 miles) or out of fuel, effectiveSpeed is 0 (background, loot stop)
  ...
  function updateRoadLoot(deltaTime, effectiveSpeed) {
    for (let i = game.roadLoot.length - 1; i >= 0; i--) {
      ...
      // Multiply by car.direction: if 1, loot moves left; if -1, loot moves right.
  ...
  // Delay loot log message slightly (500ms delay)
  ...
  // Harvest Aether Button – now adds exactly 1 aether per click
  clickButton.addEventListener("click", function() {
    const amount = 1; // Force adding exactly 1 aether
    game.aether += amount;
  ...
  fuelCarButton.addEventListener("click", function() {
    if (game.aether >= 10) {
      if (game.car.fuel <= 0) {
        ...
        fuelRanOutLogged = false; // reset flag on refuel
      }
      game.aether -= 10;
  ...
  returnHomeButton.addEventListener("click", function() {
    ...
    if (game.car.direction === 1) {
      ...
      // "Car is returning home" message now appears in pink (see addLog)
  ...
  function gameLoop() {
    ...
    if (game.car.isStuck) {
      ...
      if (game.car.direction === -1) {
        ...
        // Update loot movement while returning
  ...
  function loadGame() {
    ...
    if (savedGame) {
      ...
      // For a new game, choose weather and environment only once and save them.
  ...
  // (The harvest aether button now always adds exactly 1 aether per click)
  clickButton.addEventListener("click", function() {
    const amount = 1;
    game.aether += amount;
    game.totalAether += amount;
