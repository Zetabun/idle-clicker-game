...
def replace_until(path, start, end, new):
    """Replace from start up to but not including end, leaving end in place."""
    text = read(path)
    a = text.find(start)
    if a < 0:
...
# ---------------------------------------------------------------------------
# Release and browser cache wiring.
...
bus = read('bus.html')
if '?v=0.8.0' not in bus:
    raise SystemExit('bus.html: expected 0.8.0 cache busters')
bus = bus.replace('?v=0.8.0', '?v=0.8.1')
write('bus.html', bus)
replace_once(
    'bus.html',
    '<script src="kerbside-rail-calibration.js?v=0.8.1"></script>',
    '<script src="kerbside-rail-timebands.js?v=0.8.1"></script>\n<script src="kerbside-rail-calibration.js?v=0.8.1"></script>'
)


# ---------------------------------------------------------------------------
# Forecast calibration: consume the generated 2025 RAI0202/RAI0203 table.
cal = 'kerbside-rail-calibration.js'
sub_once(
    cal,
    r'/\* Placeholder for RAI0202 / RAI0203 once parsed:.*?const TIME_BANDS=\{\};',
...
replace_until(
    ...
    '\n/* Service class.',
    r'''function measuredBand(station,minute,direction='departures'){
...
replace_until(
    ...
    r'''function contextNote(station,minute,date){
...
# Forecast v3 now gets its category boundaries from the calibration module.
...
replace_once(
    f3,
    'calibratedDemandSignal,serviceClassSignal,calibration,easterSunday,removeLegacyFeedback,STATION_TIER,MAX_EVENT_PRESSURE',
    'calibratedDemandSignal,serviceClassSignal,calibration,scoreThresholds,easterSunday,removeLegacyFeedback,STATION_TIER,MAX_EVENT_PRESSURE'
)


# ---------------------------------------------------------------------------
# Privacy-safe local Forecast v3 validation. It stores category error only.
...
# ---------------------------------------------------------------------------
# Connection Quality v2: dynamic hub buffers and timetable-topology checks.
...
sub_once(
    tt,
    r'(function lowerBound\(list,value\)\{[^\n]+\}\n)',
    r'''\1function buildStationGraph(rows){
...
# ---------------------------------------------------------------------------
# Status: local 24-hour reliability samples per provider.
...
replace_once(
    status,
    'Checks run only when this tab is opened or you press Check again. Kerbside never exposes API keys. Authenticated third-party feeds are checked through the same Workers or published build outputs the app already uses.',
    'Checks run only when this tab is opened or you press Check again. The 24-hour reliability figures are local samples from checks made on this device, not continuous central monitoring. Kerbside never exposes API keys. Authenticated third-party feeds are checked through the same Workers or published build outputs the app already uses.'
)
replace_once(
    status,
...
replace_once(
    status,
    "function install(){if(state.installed)return true;installStyles();if(!installUi())return false;const definitions=sourceDefinitions();",
    "function install(){if(state.installed)return true;if(!state.history)state.history=readHistory();installStyles();if(!installUi())return false;const definitions=sourceDefinitions();"
)
replace_once(
    status,
    'coverageHealth,overallState,liveRailProbeState,sourceDefinitions,refresh,install',
    'coverageHealth,overallState,liveRailProbeState,historySummary,recordHistory,sourceDefinitions,refresh,install'
)
...
provider_test = 'kerbside-backend/test/train-timetable-provider.test.js'
replace_once(provider_test, "  assert.equal(provider.connectionMinimum('BHM'),12);", "  assert.equal(provider.connectionMinimum('BHM'),15);")
provider_add = r'''
...
if 'connection quality rejects literal network backtracking' not in text:
    write(provider_test, text + provider_add)
...
'''
if 'status history reports rolling local availability' not in text:
    write(status_test, text + status_add)

print('Applied Kerbside 0.8.1 connection quality, measured DfT calibration, local accuracy and status history.')
