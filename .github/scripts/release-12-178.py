...
must_replace('js/00-core.js', "const BUILD_VERSION = '12.177';", "const BUILD_VERSION = '12.178';")
must_replace('js/00-core.js', "const BUILD_NAME = 'Management Grid CSS Ownership';", "const BUILD_NAME = 'Route Readability CSS Ownership';")
must_replace('js/00-core.js', "const BUILD_ID = '12.177.0-management-grid-css-ownership';", "const BUILD_ID = '12.178.0-route-readability-css-ownership';")
...
marker = '''/* --------------------------------------------------------------------------
   Build 12.134 — compact readability floor, part two.'''
position = css.find(marker)
if position < 0:
    raise SystemExit('Build 12.134 route-readability marker missing')
block = css[position:].strip() + '\n'
required_css = (
    'Build 12.134 — compact readability floor, part two.',
    'Build 12.134 - compact readability floor, part three (second audit pass).',
    '@media (max-width: 1023px)',
    '#menuContent .league-pyramid > div > span { font-size: 11px !important; }',
    '#menuContent .development-alert-strip > button > span { font-size: 12px !important; }',
    '#menuContent .club-formation-card > em { font-size: 11px !important; line-height: 1.4 !important; }',
    '#menuContent .commercial-deal-grid > div,',
    '#menuContent .menu-info-tile { min-width: 0; }',
    '#menuContent .league-opponent-facts > div > span,',
    '#menuContent .career-armour-guide-grid > article > strong { font-size: 12px !important; }',
    '#menuContent .player-accolade-entry { min-width: 0; }',
)
if any(item not in block for item in required_css):
    raise SystemExit('Route readability declaration missing from extracted block')
if block.count('@media (max-width: 1023px)') != 2:
    raise SystemExit('Unexpected route readability media-query count')
css_path.write_text(css[:position].rstrip() + '\n', encoding='utf-8', newline='\n')
(root / 'css/route-readability.css').write_text(
    '/* Route-specific compact typography and containment ownership.\n'
    '   Extracted from the Build 12.134 tail in Build 12.178 without changing declarations. */\n\n'
    + block,
    encoding='utf-8', newline='\n',
)
...
if text.count('>12.177</b>') != 2:
    raise SystemExit('Expected exactly two visible 12.177 build labels')
text = text.replace('Strikewatch 12.177: Management Grid CSS Ownership', 'Strikewatch 12.178: Route Readability CSS Ownership', 1)
text = text.replace('12.177.0-management-grid-css-ownership', build_id)
text = text.replace('>12.177</b>', '>12.178</b>')
old_links = '''<link rel="stylesheet" href="css/game.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/management-grid.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/league-table.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/calendar-agenda.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/training-readability.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/training-programme.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/management-feedback.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/compact-readability.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/reward-reveal.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/armour-viewer.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/weapon-presentation.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/loadout-stills.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/12.161-audit-fixes.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/armoury-inventory.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/compact-navigation.css?v=12.178.0-route-readability-css-ownership" />'''
new_links = '''<link rel="stylesheet" href="css/game.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/route-readability.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/management-grid.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/league-table.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/calendar-agenda.css?v=12.178.0-route-readability-css-ownership" />
<link rel="stylesheet" href="css/training-readability.css?v=12.178.0-route-readability-css-ownership" />
...
text = text.replace('"game_css_lines_max": 31185', '"game_css_lines_max": 31080', 1)
...
text = text.replace('Build: **12.177 — Management Grid CSS Ownership**', 'Build: **12.178 — Route Readability CSS Ownership**', 1)
text = text.replace('Build ID: `12.177.0-management-grid-css-ownership`', 'Build ID: `12.178.0-route-readability-css-ownership`', 1)
text = text.replace('strikewatch-build-12.177.html', 'strikewatch-build-12.178.html', 1)
old_note = 'Build 12.177 continues SW-020 by moving the Build 12.135/12.136 management grid-track sizing into `css/management-grid.css`. `grid-auto-rows: max-content` and `align-content: start` remain unchanged, so route panels keep content-height rows without clipping or overlap. The sheet follows `game.css` before all later component layers. See `AUDIT-12.177.md`.'
new_old_note = 'Build 12.177 continues SW-020 by moving the Build 12.135/12.136 management grid-track sizing into `css/management-grid.css`. `grid-auto-rows: max-content` and `align-content: start` remain unchanged, so route panels keep content-height rows without clipping or overlap. After the 12.178 extraction, the sheet follows `route-readability.css` before all later component layers. See `AUDIT-12.177.md`.'
if old_note not in text:
    raise SystemExit('12.177 handoff note missing')
new_note = 'Build 12.178 continues SW-020 by moving the Build 12.134 route-specific compact typography floors and dense-grid containment into `css/route-readability.css`. The 11–12px label/copy floors, 1023px breakpoint and `min-width: 0` containment remain unchanged. The sheet follows `game.css` before all later component layers. See `AUDIT-12.178.md`.\n\n'
...
text = text.replace(
    'Build 12.177 owns management-route grid track sizing in `css/management-grid.css`. Keep it immediately after `game.css`, before `league-table.css`.',
    'Build 12.177 owns management-route grid track sizing in `css/management-grid.css`. Keep it after `route-readability.css`, before `league-table.css`.',
    1,
)
agents.write_text(text, encoding='utf-8', newline='\n')

must_replace('README.md', '# Strikewatch Source 12.177', '# Strikewatch Source 12.178')
must_replace('README.md', 'dist/strikewatch-build-12.177.html', 'dist/strikewatch-build-12.178.html')

project = root / 'PROJECT.md'
text = project.read_text(encoding='utf-8')
old_current = '''Build 12.177 continues the staged CSS ownership programme by moving management-
...
new_current = '''Build 12.178 continues the staged CSS ownership programme by moving the Build
...
must_replace('00-READ-FIRST-GPT.md', 'Current release: **Strikewatch Build 12.177 — Management Grid CSS Ownership**.', 'Current release: **Strikewatch Build 12.178 — Route Readability CSS Ownership**.')
...
if '## 12.178 — Route Readability CSS Ownership' not in text:
    text = header + entry + text[len(header):]
changelog.write_text(text, encoding='utf-8', newline='\n')

(root / 'AUDIT-12.178.md').write_text('''# Build 12.178 — Route Readability CSS Ownership
