...
'''
source = replace_once(source, '</style>', css + '</style>', 'css overrides')

old_board = '''        <button class="changestop" id="changeStop">Choose a different stop</button>
...
new_board = '''        <div class="board-actions">
...
source = replace_once(source, old_board, new_board, 'board information markup')
...
source = replace_once(source, old_settings, new_settings, 'settings header markup')

old_regions = '''          <div class="stats-section">
            <div class="stats-section-head"><h3>Regional coverage</h3><span id="statsAppVersion">App —</span></div>'''
new_regions = '''          <div class="stats-section gps-stats-section">
          ...
          <div class="stats-section">
            <div class="stats-section-head"><h3>Regional coverage</h3><span id="statsAppVersion">App —</span></div>'''
source = replace_once(source, old_regions, new_regions, 'gps stats markup')
...
visual = '''function visualVehiclePosition(v,now=Date.now()){
...
'''
source = replace_once(source, '/* inbound = getting closer to the town anchor */', visual + '/* inbound = getting closer to the town anchor */', 'bounded visual gps motion')
source = replace_once(source, "    const brg = isFinite(v.bearing)?v.bearing:0;\n", "    const brg = isFinite(v.bearing)?v.bearing:0;\n    const visual=visualVehiclePosition(v);\n", 'visual marker position')
...
source = replace_once(source, "  if(v && S.selected){\n    map.panTo([v.lat,v.lon],{animate:true});", "  if(v && S.selected){\n    const visual=visualVehiclePosition(v);\n    map.panTo([visual.lat,visual.lon],{animate:true});", 'focus visual position')
source = replace_once(
    source,
...
source = replace_once(source, "+'<span class=\"chip conf-'+r.confidence+'\">'+confLabel+'</span>'", "+'<span class=\"chip conf-'+r.confidence+'\" title=\"'+esc(confHelp)+'\" aria-label=\"'+esc(confLabel+': '+confHelp)+'\">'+confLabel+'</span>'", 'confidence tooltip')

board_info_js = '''function setBoardInfo(open){
  const expanded=!!open;
...
gps_stats_js = '''function renderGpsStats(){
  const now=Date.now();
...
'''
source = replace_once(source, 'function renderNetworkStats(manifest,error){', gps_stats_js + 'function renderNetworkStats(manifest,error){', 'gps stats renderer')
source = replace_once(source, "  }).join(''):'<div class=\"region-stat\"><b>No regional statistics available</b></div>';\n}", "  }).join(''):'<div class=\"region-stat\"><b>No regional statistics available</b></div>';\n  renderGpsStats();\n}", 'network stats gps refresh')
source = replace_once(source, "  if(stats){\n    if(DATA_MANIFEST) renderNetworkStats(DATA_MANIFEST,null);\n    else refreshNetworkStats(false);\n  }", "  if(stats){\n    renderGpsStats();\n    if(DATA_MANIFEST) renderNetworkStats(DATA_MANIFEST,null);\n    else refreshNetworkStats(false);\n  }", 'stats tab gps refresh')
source = replace_once(source, "  if(S.demo){el.className='settings-status good';el.textContent='Simulator active · '+dataText+' · app '+APP_VERSION;return;}\n  if(!S.proxy){el.className='settings-status';el.textContent=(S.key?'Local key mode selected':'Add your Cloudflare Worker URL')+' · '+dataText+' · app '+APP_VERSION;return;}", "  if(S.demo){S.workerOk=null;renderGpsStats();el.className='settings-status good';el.textContent='Simulator active · '+dataText+' · app '+APP_VERSION;return;}\n  if(!S.proxy){S.workerOk=null;renderGpsStats();el.className='settings-status';el.textContent=(S.key?'Local key mode selected':'Add your Cloudflare Worker URL')+' · '+dataText+' · app '+APP_VERSION;return;}", 'gps source precheck')
source = replace_once(source, "    if(!r.ok) throw new Error(j.error||('HTTP '+r.status));\n    el.className='settings-status good';el.textContent='Live Worker reachable · BODS secret '+(j.bods?'configured':'missing')+' · '+dataText+' · app '+APP_VERSION;\n  }catch(e){el.className='settings-status bad';el.textContent='Live Worker check failed: '+(e.message||'unreachable')+' · '+dataText+' · app '+APP_VERSION;}", "    if(!r.ok) throw new Error(j.error||('HTTP '+r.status));\n    S.workerOk=true;S.workerVersion=String(j.version||'');S.workerBods=!!j.bods;renderGpsStats();\n    el.className='settings-status good';el.textContent='Live Worker reachable · BODS secret '+(j.bods?'configured':'missing')+' · '+dataText+' · app '+APP_VERSION;\n  }catch(e){S.workerOk=false;S.workerVersion='';S.workerBods=false;renderGpsStats();el.className='settings-status bad';el.textContent='Live Worker check failed: '+(e.message||'unreachable')+' · '+dataText+' · app '+APP_VERSION;}", 'worker gps health stats')
