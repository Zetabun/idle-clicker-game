...
test('GB public holidays suppress typical-weekday DfT evidence',()=>{
  const c=load(),v4=c.window.__KERBSIDE_FORECAST_V4__;
  assert.equal(v4.isBankHoliday(new FixedDate('2026-08-12T12:00:00Z')),false);
  assert.equal(v4.isBankHoliday(new FixedDate('2026-08-03T12:00:00Z')),true); // Scotland summer holiday
  assert.equal(v4.isBankHoliday(new FixedDate('2026-08-31T12:00:00Z')),true); // England/Wales summer holiday
  assert.equal(v4.isBankHoliday(new FixedDate('2026-12-28T12:00:00Z')),true); // Boxing Day substitute
});
...
test('forecast passes expected-time and holiday context into measured DfT signals',()=>{
  ...
  assert.ok(seen.length);assert.equal(seen.at(-1)[4],true);assert.equal(seen.at(-1)[0].operatorCode,'XC');
});

test('same route receives different final context by journey time while ORR route prior remains route-based',()=>{
  const c=load(),trains=c.window.__KERBSIDE_TRAINS__,cal=c.window.__KERBSIDE_CALIBRATION__,v4=c.window.__KERBSIDE_FORECAST_V4__;trains.state.station=station;
...
''',encoding='utf-8')
print('Temporal regression test written')
