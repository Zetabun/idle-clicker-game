...
Build 12.146 owns baked occlusion (`staticOcclusionAt`/`applyStaticOcclusion` in `js/60-renderer-core.js`). Keep it quantised — the static batcher groups by exact material, so a continuous factor shatters merged batches. Do not enable `STATIC_WORLD_BATCHING_ENABLED` for another arena until every time-dependent draw in `drawStaticWorld` is wrapped in `setStaticWorldBatchEligibility()`; batching bakes model matrices, so animated decor would freeze. **Corrected in 12.153:** that helper *is* called, in five wrapped regions, and has been since before 12.146 — the earlier claim that it is never called was wrong. Four draws remain unwrapped and are the actual blocker: the Dune lamp glow, banner cloth and torch flame, and the coolant tank pulse. Renderer stats in `document.body.dataset` stall under a throttled animation frame — use `rendererFrameStatsForTest()` and fix the camera pose, since draw calls are view-dependent. See `AUDIT-12.146.md`.
