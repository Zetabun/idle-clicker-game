...
Build 12.235 owns the club profile route and club-name links. **Register every new route in `menuTabMeta` as well as `menuSections`** — `setMenuRoute()` returns false for anything missing there and leaves the previous page rendered with no error. **Never edit a fixture row in `js/37-league.js` alone**: `js/52-season-narratives.js` replaces `renderLeagueFixtures` outright rather than delegating, so that copy is dead for the fixtures route. Keep `selectedLeagueClubId` as module state — persisting a viewed club would bump the save schema for a view position. Keep club links inside any element an owned stylesheet already sizes, and keep the `#menuContent` prefix, or `.menu-shell button`'s 7px fallback takes them. **Do not set `font-size` on a `small` inside `#menuContent`** — `game.css` owns it with the flag in both bands and `12.161-audit-fixes.css` again below 1100px, so the declaration is dead. **Verify against `dist/strikewatch-build-<version>.html?cb=<n>`, not `index.html`**: the development page requests the bundle at `?v=<BUILD_ID>`, so rebuilds within one version serve a cached bundle and a working change can look broken. See `AUDIT-12.235.md`.

Build 12.234 owns the Board Expectations surface and the compact MUST RESPOND header. **Two rules in this project are known to be outranked by the Command Skin: `.command-today-panel` and `.board-expectations-panel`.** Both must colour themselves through `--skin-panel-hi`/`--skin-panel-lo`; a `background` declaration on either silently loses to `#menuContent > section[class]`. Assume any new full-width panel has the same problem and check the computed value rather than the authored one. Keep the objectives grid flex-wrap — three cards never divide into two columns and `auto-fit` reserves a dead cell. **Never lay out `.club-must-respond-body > header`**: it restates the summary directly above it and is hidden on both targets. `game.css`'s `.club-must-respond-strip > header` block is dead code kept deliberately — retargeting it at the body header reintroduces 7px type that fails the compact floors. **The CSS-debt gate counts raw string occurrences across the combined stylesheets, comments included, so writing the override-flag token in prose spends budget exactly as a declaration does** — say "flagged declaration" instead. See `AUDIT-12.234.md`.

Build 12.233 owns Operations Today presentation, superseding the 12.230 and 12.231 notes below where they conflict. **Keep the four tone states unequal in weight** — that asymmetry is the whole point of the surface, and making them uniform is exactly the fault this build fixed. A state is one decision: set `--today-accent`, `--today-wash`, `--today-band` and `--today-cap` together, never one of them alone. **Keep the status value in a filled band flush with the card floor and do not reintroduce the reserved two-line `min-height`** — it existed only because a bottom-anchored *rule* has to align across five columns; a band does not, and `31 AUG 2026 · 23D` holds one line at every width including 320px. **The top cap must stay `::before` and the chevron `b::after`**: the skin flags `border` on a bare `button` (12.132), so a real border there costs flagged declarations against a budget with one slot left in the project. **Card headlines are sentence case** — 12.231's uppercase rule is reversed for these strings and only these: they are wrapped sentences, and the all-caps department label above each one already carries the command-chrome voice. Every other kicker on the surface stays 900-weight caps, and anything inside these buttons still needs an explicit `font-weight` or it silently inherits 900. Add no media query and no `!important` here; verify against the previous artifact rather than against zero, because `mobileInterfaceAuditForTest()` carries one long-standing overflow that is not this panel's and `typographyConsistencyForTest()` carries a pre-existing `tutorialReadable` failure. See `AUDIT-12.233.md`.

Build 12.232 owns management status anchoring. **Never change `left`, `right` or `inset` on `.management-status` without setting `transform` in the same rule.** The base centring in `css/management-feedback.css` is `left: 50%` plus `translate(-50%, …)`; overriding only the position half leaves the translate applying to a layout it was never meant for, which is exactly how a right-anchored toast ended up 210px to the left of its anchor. Keep the transform reset in `css/12.161-audit-fixes.css` alongside its `inset` rule. See `AUDIT-12.232.md`.

Build 12.231 owns Operations Today typography and contrast. **Never leave text inside these cards without an explicit `font-weight`** — `game.css` sets `button { font-weight: 900 }` and unweighted copy silently becomes the heaviest text in the card, which reads as a typeface change rather than a weight one. Follow the measured house scale: headlines 700, all-caps kickers 900-950, supporting copy 400. Keep the resting card at the 13% tone wash and hover at 21%; the darker 12.230 resting state made hover look like the correct default. Lift the card edge with the inset `box-shadow` ring, never `border-color` — the skin flags `border` on `button` and the important budget is full at 2269/2270, so that route fails the debt gate. Card titles stay uppercase. See `AUDIT-12.231.md`.

Build 12.230 owns Operations Today presentation in `css/compact-navigation.css` and its markup in `js/36-team-management.js`. Keep the layout flex-wrap, never `auto-fit` grid: five columns do not divide evenly and a grid orphans the fifth into dead space. Keep the `.command-today-panel` prefix on every compact override — this file declares media queries before base rules, so an unprefixed override loses on source order. Keep the panel darkening via `--skin-panel-hi`/`--skin-panel-lo` rather than a `background` declaration the Command Skin outranks, and keep the `!important` on the card background, which a bare `button` rule requires. Keep the status footer's two-line `min-height` or a wrapping value pulls its accent rule out of alignment. Add no media queries here; the debt gate sits at 484/484 and important declarations at 2268/2270. Verify the responsive matrix in situ and check `mobileInterfaceAuditForTest()` against the previous artifact rather than against zero — League and Profile carry two long-standing overflows that are not this panel's. See `AUDIT-12.230.md`.
