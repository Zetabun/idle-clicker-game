...
function sourceDefinitions(){
  ...
  return [
    ...
    {...byId['huxley2'],probe:async source=>{const r=await request(`${HUXLEY_PRIMARY}/departures/BHM/1`,{type:'json',headers:{Accept:'application/json'}});if(!r.response.ok)return result(source,'down',`Fallback returned HTTP ${r.response.status}.`,r.latency);return result(source,'healthy','Fallback is reachable and ready if the official source fails.',r.latency,'Standby ready');}},
    {...byId['huxley'],probe:async source=>{const r=await request(`${HUXLEY_SECONDARY}/departures/BHM/1`,{type:'json',headers:{Accept:'application/json'}});if(!r.response.ok)return result(source,'down',`Fallback returned HTTP ${r.response.status}.`,r.latency);return result(source,'healthy','Secondary fallback is reachable.',r.latency,'Standby ready');}},
    ...
    {...byId['osm-tiles'],probe:async source=>{const latency=await imageProbe('https://tile.openstreetmap.org/0/0/0.png');return result(source,'healthy','Fallback basemap tile loaded.',latency,'Standby ready');}},
